...
object HairMaskRefiner {
    ...
    fun refineForTryOn(mask: HairMask, points: List<LandmarkPoint>): SoftHairMask {
        ...
        val conf = mask.hairConfidence
        if (conf != null && conf.size == w * h) {
            for (i in raw.indices) {
                val c = (conf[i].toInt() and 0xFF) / 255f
                // Follow segmentation boundary — drop weak/noise, keep soft fringe for feathering.
                raw[i] = when {
                    c < 0.32f -> 0f
                    c > 0.78f -> 1f
                    else -> (c - 0.32f) / 0.46f
        ...
        attenuateOutsideHeadSilhouette(raw, w, h, points)
        morphOpen(raw, w, h)
        featherEdges(raw, w, h, radius = 3)
    ...
    /**
     * Foreground keep-sharp weights for portrait background blur.
     * 255 = keep original (face/head/person), 0 = apply background blur.
     * Uses MediaPipe face oval + expanded head envelope — not a tight circle that clips the face.
     */
    fun buildPortraitForegroundMask(points: List<LandmarkPoint>, w: Int, h: Int): ByteArray {
        val mask = ByteArray(w * h)
        if (points.size <= FaceShapeClassifier.CHIN) {
            mask.fill(255.toByte())
            return mask
        }

        fillExpandedHeadEnvelope(mask, points, w, h)
        fillFaceOvalInterior(mask, points, w, h)
        featherMaskEdges(mask, w, h, radius = 4)
        return mask
    }

    private fun fillFaceOvalInterior(mask: ByteArray, points: List<LandmarkPoint>, w: Int, h: Int) {
        ...
        val topInset = top.y + faceH * 0.06f
    ...
    /** Head/shoulders envelope — wider than the face so ears, hair, and glasses stay sharp. */
    private fun fillExpandedHeadEnvelope(
        mask: ByteArray,
        ...
        val faceH = (chin.y - top.y).coerceAtLeast(0.08f)
        val cy = top.y + faceH * 0.42f
        val rx = ((right.x - left.x) * 1.05f).coerceAtLeast(0.12f)
        val ry = (faceH * 2.35f).coerceAtLeast(0.18f)
    ...
    private fun featherMaskEdges(mask: ByteArray, w: Int, h: Int, radius: Int) {
        if (radius <= 0) return
        val src = mask.copyOf()
        for (y in 0 until h) {
            val row = y * w
            for (x in 0 until w) {
                ...
                for (dy in -radius..radius) {
                    ...
                    for (dx in -radius..radius) {
                        ...
                        sum += src[sy * w + sx].toInt() and 0xFF
                        count++
                    }
                }
                mask[row + x] = (sum / count).toByte()
            }
        }
    }

    /** Soft head silhouette — attenuate mask outside hair region, not a hard rectangle. */
    private fun attenuateOutsideHeadSilhouette(
        weights: FloatArray,
        w: Int,
        h: Int,
        points: List<LandmarkPoint>
        ...
        val faceH = (chin.y - top.y).coerceAtLeast(0.08f)
        val cx = (left.x + right.x) * 0.5f
        val crownY = top.y - faceH * 0.35f
        val maxY = chin.y + faceH * 0.12f
        val rx = faceW * 0.72f
        val ry = (maxY - crownY) * 0.55f + faceH * 0.25f
        val cy = crownY + (maxY - crownY) * 0.42f
        ...
        for (y in 0 until h) {
            ...
            for (x in 0 until w) {
                val nx = (x + 0.5f) / w
                if (ny > maxY + faceH * 0.08f || ny < crownY - faceH * 0.25f) {
                    weights[row + x] = 0f
                    continue
                }
                val dx = (nx - cx) / rx
                ...
                if (dist > 1.35f) {
                    ...
                    val falloff = 1f - ((dist - 1f) / 0.35f).coerceIn(0f, 1f)
                    weights[row + x] *= falloff
                }
            }
        }
    }
    ...
    private fun toAlphaBytes(weights: FloatArray): ByteArray {
        ...
        for (i in weights.indices) {
            // Smooth alpha from segmentation weight — avoids flat solid plates.
            val w = weights[i]
            val a = (w * w * 185f).toInt().coerceIn(0, 185)
            out[i] = a.toByte()
        }
        return out
    }
